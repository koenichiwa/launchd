...
impl CalendarInterval {
    pub fn with_minute(mut self, minute: u8) -> Result<Self, Error> {
        if minute > 59 {
            Err(Error::CalendarFieldOutOfBounds(
                CalendarIntervalField::Minute,
                minute,
            ))
        } else {
